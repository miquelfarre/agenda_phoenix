...
def create_invitations(db, private_users, private_events, public_events, recurring_events):
    ...
    for i, user in enumerate(all_users[10:25]):
        ...
        interactions.append(EventInteraction(
            ...
            status=status,
            invited_by_user_id=sonia.id,
            cancellation_note="No puedo ir, lo siento" if status == "rejected" else None
        ))
    ...
    interactions.append(EventInteraction(
        ...
        personal_note="Ver si puedo librar el viernes"
    ))
    ...
    interactions.append(EventInteraction(
        ...
        status="rejected",
        invited_by_user_id=miquel.id,
        cancellation_note="Tengo otro plan ese fin de semana"
    ))
    ...
    for user in all_users[10:15]:
        interactions.append(EventInteraction(
            ...
            user_id=user.id,
            interaction_type="invited",
            status="accepted" if user.id % 2 == 0 else "pending",
            invited_by_user_id=ada.id
        ))
    ...
    interactions.append(EventInteraction(
        event_id=event_clasico.id,
        user_id=sonia.id,
        interaction_type="invited",
        status="rejected",
        is_attending=True,  # Rechaza invitación pero va sola
        invited_by_user_id=miquel.id,
        cancellation_note="Prefiero ir por mi cuenta, gracias"
    ))
    ...
    for i, user in enumerate([sonia, miquel, ada] + all_users[10:32]):
        ...
        interactions.append(EventInteraction(
            ...
            interaction_type="invited",
            status=status,
            invited_by_user_id=owner_running.id
        ))
