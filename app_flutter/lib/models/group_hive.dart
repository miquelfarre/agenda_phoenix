...
class GroupHive extends HiveObject {
  ...
  @HiveField(7)
  final List<String?> memberContactNames;
  ...
  Map<String, dynamic> toJson() {
    ...
    for (int i = 0; i < memberIds.length; i++) {
      members.add({
        'id': memberIds[i],
        'instagram_name': i < memberNames.length ? memberNames[i] : null,
        'contact_name': i < memberContactNames.length ? memberContactNames[i] : null,
        'is_public': i < memberIsPublic.length ? memberIsPublic[i] : false,
      });
    }
  ...
  static GroupHive fromJson(Map<String, dynamic> json) {
    ...
    for (final member in members) {
      if (member is Map<String, dynamic>) {
        ...
        memberContactNames.add(member['contact_name'] as String?);
    ...
    return GroupHive(
      ...
      createdAt: json['created_at'] != null
          ? (json['created_at'] is String
                ? DateTimeUtils.parseAndNormalize(json['created_at'] as String)
                : json['created_at'])
          : DateTime.now(),
      memberIds: memberIds,
  ...
  bool get isCreatedOffline =>
      isOptimistic == true && effectiveClientTempId != null;
