...
class CountryService {
  ...
  static final Map<String, Country> _countryMap = {
    for (var country in _countries) country.code: country,
  };
  ...
  static List<Country> searchCountries(String query) {
    ...
    return _countries.where((country) {
      return country.name.toLowerCase().contains(lowerQuery) ||
          country.nameEn.toLowerCase().contains(lowerQuery) ||
  ...
  static List<Country> getCountriesByRegion(String region) {
    switch (region.toLowerCase()) {
      ...
      case 'europa':
        return _countries
            .where((c) => ['ES', 'FR', 'DE', 'IT', 'GB', 'PT'].contains(c.code))
            .toList();
      case 'america':
      case 'américa':
        return _countries
            .where(
              (c) =>
                  ['US', 'CA', 'MX', 'BR', 'AR', 'CO', 'CL'].contains(c.code),
            )
            .toList();
      case 'asia':
        return _countries
            .where((c) => ['JP', 'CN', 'IN', 'KR'].contains(c.code))
            .toList();
      case 'oceania':
